...
import { carOptions, BASE_PRICE, calculatePrice } from '../utilities/calcPrice'
...
const CreateCar = () => {
    ...
    return (
        ...
        <select value={color} onChange={e => setColor(e.target.value)}>
            {carOptions.color.map(o => <option key={o.name}>{o.name} (+${o.price})</option>)}
        </select>

        <label>Wheels</label>
        <select value={wheels} onChange={e => setWheels(e.target.value)}>
            {carOptions.wheels.map(o => <option key={o.name}>{o.name} (+${o.price})</option>)}
        </select>

        <label>Interior</label>
        <select value={interior} onChange={e => setInterior(e.target.value)}>
            {carOptions.interior.map(o => <option key={o.name}>{o.name} (+${o.price})</option>)}
        </select>

        <label>Engine</label>
        <select value={engine} onChange={e => setEngine(e.target.value)}>
            {carOptions.engine.map(o => <option key={o.name}>{o.name} (+${o.price})</option>)}
        </select>
